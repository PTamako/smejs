    ...
    jet.extend(jeDatePick.prototype, {
        ...
        dateTemplate: function () {
            ...
            var lyPrev = '<em class="yearprev yprev jedatefont" @on="yearBtn(lprev,' + aowArr[0] + ')">&#xed6c2;</em>',
                ...
                mPrev = '{% if(dlen>2){ %}<em id="monthprev" data-m={%=daytit[i].MM%} class="monthprev  mprev jedatefont" @on="monthBtn(mprev,{%=daytit[i].YYYY%}-{%=daytit[i].MM%})">&#xed602;</em>{% } %}',
                mNext = '{% if(dlen>2){ %}<em id="monthnext" data-m={%=daytit[i].MM%} class="monthnext  mnext jedatefont" @on="monthBtn(mnext,{%=daytit[i].YYYY%}-{%=daytit[i].MM%})">&#xed605;</em>{% } %}';
            //循环年的模板
            ...
            var daysHtml = '<table class="daystable days{%= i==0 ? "left":"right"%}" style="display:{%=day ? "block":"none"%};"><thead><tr>' +
                ...
                '<tr>{% for(var d=0;d<=41;d++){ %}<td @on="daysClick({%=daylist[i][d].ymd%})" class="{%=daylist[i][d].style%}" ymd="{%=daylist[i][d].ymd%}" >{%=daylist[i][d].day%}</td>{% if((d+1)%7==0){ %} </tr>{% } %} {% } %} </tbody></table>';
            // //循环时间模板
            // var daysHtml = '<table class="daystable days{%= i==0 ? "left":"right"%}" style="display:{%=day ? "block":"none"%};"><thead><tr>' +
            // '{% for(var w=0;w<lang.weeks.length;w++){ %} <th>{%=lang.weeks[w]%}</th> {% } %}</tr></thead><tbody>' +
            // '<tr>{% for(var d=0;d<=41;d++){ %}<td class="{%=daylist[i][d].style%}" ymd="{%=daylist[i][d].ymd%}" >{%=daylist[i][d].day%}</td>{% if((d+1)%7==0){ %} </tr>{% } %} {% } %} </tbody></table>';
        //循环时间模板
            ...
            var ymtitHtml = (function () {
                ...
                if (that.dlen == 1) {
                    ...
                    ymtitStr = '<span class="ymbtn" >{%=daytit[i].YYYY%}' + ytxt + '</span>' +
                        '<span class="ymbtn" >{%=daytit[i].MM%}' + mtxt + '</span>'
                        ;
                }
                return ymtitStr;
            })();
        ...
        methodEventBind: function () {
            ...
            jet.extend(that, {
                ...
                monthBtn: function (type, val) {
                    ...
                    opts.toggle && opts.toggle({ elem: that.valCell, val: dateVal, date: exarr[0] });
                    var newMonth = PNYM.m < 10 ? "0" + PNYM.m : PNYM.m
                    var newtime = PNYM.y + "-" + newMonth
                    // console.log(newtime)

                    var tjpostData = { date: newtime }
                    var tjurl = 'partyCloudLive/listMonthCount';
                    Method.$post(window.BaseUrl + tjurl, tjpostData, function (data) {
                        if (+data.status === 1) {
        ...
        eachDays: function (yd, md, ds, idx) {
            ...
            var isfestival = function (y, m, d) {
                ...
                if (opts.festival == true && lang.name == "cn") {
                    ...
                    var ymd = y + "-" + nm + "-" + nd
                    festivalStr = '<i class=dateicon'+d+' data-id='+d+'></i><p data-ymd=' + ymd + '  class="nolunar">' + d + '</p>';
                }
                return festivalStr;
            };
            //判断是否在限制的日期之中
